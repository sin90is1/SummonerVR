...
namespace keras2cpp
{
    namespace layers
    {
        ...
        , activation_(file)
        , return_sequences_(static_cast<unsigned>(file) != 0) {}
        // linux compile error: comparison between NULL and non-pointer ('unsigned int' and NULL)
...
#else
            auto out = Tensor::empty<size_t>({ steps, out_dim });
            Tensor last({ 1, out_dim });
#endif
