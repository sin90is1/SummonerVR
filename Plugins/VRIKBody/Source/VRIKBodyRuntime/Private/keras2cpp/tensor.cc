...
namespace keras2cpp {
    ...
    Tensor Tensor::unpack(size_t row) const noexcept {
        ...
        size_t pack_size = std::accumulate<std::vector<size_t>::const_iterator, size_t>(dims_.begin() + 1, dims_.end(), static_cast<size_t>(0u));
    ...
    Tensor& Tensor::operator+=(const Tensor& other) noexcept {
        ...
        std::transform<std::vector<float>::const_iterator, std::vector<float>::const_iterator>(begin(), end(), other.begin(), begin(), std::plus<float>());
        return *this;
    }

    Tensor& Tensor::operator*=(const Tensor& other) noexcept {
        kassert(dims_ == other.dims_);
        std::transform<std::vector<float>::const_iterator, std::vector<float>::const_iterator>(begin(), end(), other.begin(), begin(), std::multiplies<float>());
        return *this;
    }
